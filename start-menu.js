...
<div id="wrapper">
  <div id="help-popup">
    <div id="help-title" onclick="showHelpScreen()">
...
<script>
  ...
  let checkVar = false;
  var bgColor = "#000";
  function updateIframeSize(hvp, wvp) {
    parent.postMessage({ type: "resize", expanded, hvp: hvp, wvp: wvp }, "*");
  }

  function setWrapperBgColor(color) {
  ...
  window.addEventListener("message", function (e) {
    ...
    if (e.data.type === 'mousedata') {
      ...
      if (checkVar) {
        updateIframeSize(hvp, wvp);
        setWrapperBgColor(bgColor);
      }
    }
    ...
    if (e.data.widget) {
      property = e.data.widget.property;
    } else {
      property = e.data.property;
    }
    inputData = property;

    if (property?.apperance?.bg_color) {
      bgColor = property.apperance.bg_color;
      if (checkVar) {
        updateIframeSize(hvp, wvp);
        setWrapperBgColor(bgColor);
      }
...
<style>
  ...
  *::-ms-scrollbar-thumb {
  ...
  label[for="welcome-checkbox"] {
    /* Add your styles for the "Welcome" label here */
    cursor: pointer;
    justify-content: center;
    padding-top: 14px;
  }

  label[for="welcome-checkbox"]>input[type="checkbox"] {
    /* Add your styles for the checkbox inside the "Welcome" label here */
    cursor: pointer;
    justify-content: center;
    padding-top: 14px;
  }
  ...
  label[for^="agree-checkbox"] {
    /* Add your styles for the "Agreement" label here */
    cursor: pointer;
    color: var(--character-title-85, rgba(0, 0, 0, 0.85));
  ...
  label[for^="agree-checkbox"]>input[type="checkbox"] {
    /* Add your styles for the checkbox inside the "Agreement" label here */
    cursor: pointer;
    justify-content: center;
    padding-top: 24px;
  }
...
<div id="wrapper">
  <div id="header"></div>
  <div id="content">
    <section id="main-section" screen="1" style="display: none;">
      <div id="main-content">
      </div>
      <img class="" id="help-img">
      <input type="hidden" id="agree-checkbox-hidden" value="">
      <span id="agree-checkbox-span">
      </span>
      <label for="welcome-checkbox">
    ...
    <div class="prev-next">
      <button id="prev-btn" class="main-btn" style="display: none;">
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="21" viewBox="0 0 20 21" fill="none">
          <path d="M16.875 10.5H3.125" stroke="white" stroke-linecap="round" stroke-linejoin="round" />
          <path d="M8.75 4.875L3.125 10.5L8.75 16.125" stroke="white" stroke-linecap="round" stroke-linejoin="round" />
        </svg>Prev</button>
      <div id="pagination"></div>
      <button id="next-btn" class="main-btn" page="" style="display: none;">Next<svg xmlns="http://www.w3.org/2000/svg"
          width="20" height="21" viewBox="0 0 20 21" fill="none">
          <path d="M3.125 10.5h13.75m-5.625 5.625 5.625-5.625-5.625-5.625" stroke="#fff" stroke-linecap="round"
            stroke-linejoin="round" />
        </svg></button>
      <button id="use-btn" class="use-btn" page="" style="display: none;">Start to use</button>
    </div>
  </div>
</div>
...
<script>
  ...
  addEventListener("message", (e) => {
    ...
    document.documentElement.style.width = wvp + "px";
    document.documentElement.style.height = hvp + "px";

    document.body.style.width = wvp + "px";
    document.body.style.height = hvp + "px";
  ...
  function closeScreen() {
    parent.postMessage({ type: "closeModal", msg: 'Msg from Modal: ' + Math.random() }, "*");
  }

  function modifyContent(content) {
    // Set options to sanitize the HTML
    const options = {
      sanitize: true,
    ...
    let parsedContent = marked.parse(content, options);

    // Find all anchor tags in the parsed content
    const regex = /<a(.*?)>/g;
    parsedContent = parsedContent.replace(regex, function (match, p1) {
  ...
  function handleModalData(data) {
    ...
    if (data.apperance?.bg_color) {
      bgColor = data.apperance.bg_color;
      const bgOpacity = 0.7; // Opacity value ranges from 0 (fully transparent) to 1 (fully opaque)

      const red = parseInt(bgColor.slice(1, 3), 16);
      const green = parseInt(bgColor.slice(3, 5), 16);
      const blue = parseInt(bgColor.slice(5, 7), 16);

      const rgbaColor = 'rgba(' + red + ', ' + green + ', ' + blue + ', ' + bgOpacity + ')';

      document.documentElement.style.backgroundColor = rgbaColor;
      document.body.style.backgroundColor = rgbaColor;
    }
    let currentPage = 0; // keeps track of the current page index

    if (data.page_setting) {
      const sortedData = data.page_setting;
      ...
      function showPage(pageIndex) {
        ...
        const page = sortedData[pageIndex];

        let paginationDiv = document.getElementById('pagination');
        paginationDiv.innerHTML = '';

        // Create pagination for plugin
        for (var i = 0; i <= sortedData.length - 1; i++) {
          ...
          // Set fill color based on activePage
          var fillColor = (i === pageIndex) ? primaryColor : '#D9D9D9';
        ...
        const spanAgreeCheckbox = document.getElementById("agree-checkbox-span");
        const helpImg = document.getElementById("help-img");
        const checkboxWelcome = document.getElementById("welcome-checkbox");
        const labelElementWelcome = document.querySelector('label[for="welcome-checkbox"]');
        const checkboxHidden = document.getElementById("agree-checkbox-hidden");
        ...
        const svgPrev = '<svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M16.875 10.5H3.125M8.75 4.875 3.125 10.5l5.625 5.625" stroke="#fff" stroke-linecap="round" stroke-linejoin="round"/></svg>';

        const svgNext = '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="21" viewBox="0 0 20 21" fill="none"><path d="M3.125 10.5h13.75m-5.625 5.625 5.625-5.625-5.625-5.625" stroke="#fff" stroke-linecap="round" stroke-linejoin="round"/></svg>'
        ...
        function hidePrevBtn() {
          ...
          prevBtn.style.border = "0px solid transparent";
          prevBtn.disabled = true;
          prevBtn.style.cursor = "auto";
          prevBtn.innerHTML = svgPrevTransparent + "Prev";
        }

        function showPrevBtn() {
          prevBtn.style.cursor = "pointer";
          prevBtn.style.display = "flex";
          prevBtn.style.backgroundColor = primaryColor;
          prevBtn.style.color = "#fff";
          prevBtn.disabled = false;

          prevBtn.innerHTML = svgPrev + 'Prev';
        }
        ...
        function showNextBtn() {
          ...
          nextBtn.innerHTML = 'Next' + svgNext;
        }

        function showNextBtnDisable() {
          nextBtn.disabled = true;
          nextBtn.style.backgroundColor = "transparent";
          nextBtn.style.color = "#9D9D9D";
          nextBtn.style.outline = "1px solid #9D9D9D";
          const svgNext = '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="21" viewBox="0 0 20 21" fill="none"><path d="M3.125 10.5h13.75m-5.625 5.625 5.625-5.625-5.625-5.625" stroke="#9D9D9D" stroke-linecap="round" stroke-linejoin="round"/></svg>'
          nextBtn.innerHTML = 'Next' + svgNext;
        }

        function showStartToUseBtn() {
          useBtn.style.display = "flex";
          useBtn.disabled = false;
          useBtn.style.backgroundColor = primaryColor;
          useBtn.style.color = "#fff";
          useBtn.style.outline = "none";
          useBtn.innerHTML = 'Start to use';
          useBtn.style.boxSizing = "border-box";
        }

        function setStartToUseBtnActive() {
          useBtn.disabled = false;
          useBtn.style.backgroundColor = primaryColor;
          useBtn.style.color = "#fff";
          useBtn.style.display = "flex";
        ...
        function setStartToUseBtnDisabled() {
          useBtn.disabled = true;
          useBtn.style.backgroundColor = "transparent";
          useBtn.style.color = "#9D9D9D";
          useBtn.style.outline = "1px solid #9D9D9D";
        }

        function displayWelcomeCheckbox(arg = "none") {
          //none or flex
          checkboxWelcome.style.display = arg;
          labelElementWelcome.style.display = arg;
        }
        ...
        switch (page.page_type) {
          case "md_page":
            ...
            if (pageIndex === 0 && pageIndex === sortedData.length - 1) {
              //handle 1st page is last page
              //hide prev btn
              hidePrevBtn();

              //show start to use button
              showStartToUseBtn();
            ...
            //end handle button show


            if (page.md_content) {
              mainContent.innerHTML = modifyContent(page.md_content);
            } else {
              mainContent.innerHTML = '<div class="welcome-title-error">データを入力してからリロードしてページを表示してください。</div>';;
            }
          ...
          case "agreement_page":
            ...
            spanAgreeCheckbox.style.display = "flex";
            spanAgreeCheckbox.innerHTML = "";
            const checkboxInput = document.createElement("input");
            checkboxInput.id = "agree-checkbox" + pageIndex;
            checkboxInput.type = "checkbox";
            checkboxHidden.getAttribute("state" + page.id) === "true" ? checkboxInput.checked = true : checkboxInput.checked = false;
            checkboxInput.classList.add("agree-checkbox");
            ...
            const labelText = document.createTextNode("\u00A0同意する");
            label.appendChild(checkboxInput);
            label.appendChild(labelText);

            spanAgreeCheckbox.appendChild(label);
            ...
            if (pageIndex === 0 && pageIndex === sortedData.length - 1) {
              ...
              checkboxInput.addEventListener('change', function () {
                if (this.checked) {
                  ...
                  setStartToUseBtnActive();
                  checkboxHidden.setAttribute("state" + page.id, "true");
                } else {
                  // Perform actions when the checkbox is unchecked
                  setStartToUseBtnDisabled();
                  checkboxHidden.setAttribute("state" + page.id, "false");
                }
              });

              checkboxInput.checked ? setStartToUseBtnActive() : setStartToUseBtnDisabled()
              ...
              //show next button
              showNextBtn();
              nextBtn.setAttribute("page", pageIndex);
              ...
              checkboxInput.checked ? showNextBtn() : showNextBtnDisable()
              ...
              //show start to use button
              showStartToUseBtn();
              useBtn.setAttribute("page", pageIndex);
              ...
              //hide start to use btn
              useBtn.style.display = "none";

              //select checbox with page
              checkboxInput.addEventListener('change', function () {
                if (this.checked) {
                  checkboxHidden.setAttribute("state" + page.id, "true");
                  // Perform actions when the checkbox is checked
            ...
            if (page.agree_content) {
              mainContent.innerHTML = modifyContent(page.agree_content);
            } else {
              mainContent.innerHTML = '<div class="welcome-title-error">データを入力してからリロードしてページを表示してください。</div>';;
            }
          ...
          default:
            ...
            //add detail content to welcome page
            const welcomeTitle = page.page_title;
            const welcomeDescription = page.page_description;
            ...
            let blobbg = new Blob([defaultbg], { type: 'image/svg+xml' });
            let bgImageDefault = URL.createObjectURL(blobbg);

            const welcomeMediaType = page.media_type ? page.media_type : "image_type";
            const welcomeImageUrl = page.image_url || bgImageDefault;

            //change url image to default image url

            const welcomeVideoUrl = page.video_url;
            const thumnailVideoUrl = page.thumbnail_video_url;

            const mediaWr = document.createElement('div');
            mediaWr.classList.add("welcome-img");
            ...
            if (welcomeMediaType === "video_type") {
              if (welcomeVideoUrl) {
                renderReactPlayer(mediaDiv, {
                  url: welcomeVideoUrl,
                  width: "100%",
                  height: "100%",
                  light: thumnailVideoUrl,
                  playing: true,
                  controls: true,
                  playsinline: true
                });
              } else {
                tempDiv = createImageElement(bgImageDefault);
                mediaDiv.appendChild(tempDiv);
              }
            } else {
              tempDiv = createImageElement(welcomeImageUrl);
              mediaDiv.appendChild(tempDiv);
            }

            function createImageElement(imageUrl) {
              let tempDiv = document.createElement('img');
              tempDiv.src = imageUrl;
              tempDiv.style.width = "100%";
              tempDiv.style.height = "100%";
              tempDiv.style.objectFit = "cover";
              return tempDiv;
            }

            mediaWr.appendChild(mediaDiv);
            //create div #media

            mainContent.appendChild(welcomeTitleDiv);
            mainContent.appendChild(welcomeDescriptionDiv);
            mainContent.appendChild(mediaWr);
      ...
      function nextPage() {
        if (currentPage < data.page_setting.length - 1) {
          currentPage++;
          showPage(currentPage);
        }
      }
    ...
    document.getElementById("use-btn").addEventListener("click", () => {
      // Handle when the button text is "Start to use"
      if (document.getElementById("welcome-checkbox").checked) {
        $.cookie('plateauCookies', 'on', {
          expires: 7,
          path: '/'
        });
        cookie = $.cookie('plateauCookies');
        parent.postMessage({ type: "saveCookie", cookie }, "*")
        $('#wrapper').hide();
      }
      closeScreen();
    });
...
let isCloseCheck = false;
let isSetFullScreen = false;

reearth.on("message", (msg) => {
if (msg.type === "saveCookie") {
reearth.clientStorage.setAsync("cookies", msg.cookie);
}

reearth.clientStorage.getAsync("cookies").then((cookie) => {
if (cookie === "on") {
...
if (!isSetFullScreen) {
reearth.ui.postMessage({ setFullScreen: "set full" }, "*");
...
if (msg.type === "resize") {
reearth.ui.resize?.(
msg.expanded ? msg.wvp : msg.wvp,
msg.expanded ? msg.hvp : msg.hvp,
